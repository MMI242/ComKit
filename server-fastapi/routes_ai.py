...
async def generate_recipe(
    ...
    if not is_ai_configured():
        raise HTTPException(
            status_code=503, detail="AI service is not configured on the server"
        )
    ...
    try:
        ...
        context = await ai_state_machine.process_request(
            user_id=current_user.id, ingredients=prompt, request_id=request_id
        )
        ...
        if context.current_state == AIRequestState.COMPLETED:
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to generate recipe: {error_msg}"
            )
...
async def get_active_requests(
    ...
    try:
        ...
        user_requests = {
            req_id: status
            for req_id, status in all_requests.items()
            if status.get("user_id") == current_user.id
        }
...
async def cancel_request(
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(
                status_code=400, detail="Cannot cancel completed request"
            )
...
@log_execution_time
async def get_observers_status(
    current_user: User = Depends(get_current_user_from_cookies_or_token),
):
    ...
    try:
        ...
        return {
            ...
            "observers": [
                obs.get_observer_id() for obs in ai_request_notifier._observers
            ],
            "metrics": metrics,
            "recent_events": [
                {
        ...
        return {
            ...
            "derived_metrics": {
                ...
                "retry_rate_percent": round(
                    (metrics["retry_count"] / max(metrics["total_requests"], 1)) * 100,
                    2,
                ),
                "fallback_rate_percent": round(
                    (metrics["fallback_count"] / max(metrics["total_requests"], 1))
                    * 100,
                    2,
                ),
            },
