...
def get_user_requests(
    ...
    if type not in ["incoming", "outgoing"]:
        raise HTTPException(
            status_code=400, detail="Type must be 'incoming' or 'outgoing'"
        )
...
async def update_request_status(
    ...
    if ENABLE_NOTIFICATIONS:
        try:
            ...
            await notification_manager.send_personal_notification(
                notification_recipient_id, notification
            )
    ...
    return RequestResponse(
        id=request.id,
        item=RequestItemInfo(
            id=item.id, name=item.name, unit=item.unit, thumbnail_url=item.thumbnail_url
        ),
        requester=RequestUser(
        ...
        owner=RequestUser(id=owner.id, username=owner.username, name=owner.name)
        if is_requester
        else None,
