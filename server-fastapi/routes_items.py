...
from schemas import ItemListResponse, ItemResponse, ItemOwner, RequestCreate
...
async def request_item(
    ...
    if request_data.requested_qty <= 0:
        raise HTTPException(
            status_code=400, detail="Requested quantity must be greater than 0"
        )

    if request_data.requested_qty > item.remaining_qty:
        raise HTTPException(
            status_code=400,
            detail=f"Insufficient quantity. Only {item.remaining_qty} available",
        )
