...
class OllamaProvider(AIProvider):
    ...
    def __init__(self):
        self.api_url = config_manager.get(
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                response = await client.post(
                    f"{self.api_url}/api/generate", headers=headers, json=payload
                )
...
class AIProxy:
    ...
    async def generate_recipe(
        ...
        if target_provider in self.providers:
            try:
                if await self._is_provider_healthy(target_provider):
                    ...
                    result = await self.providers[target_provider].generate_recipe(
                        ingredients, model or ""
                    )
                    return self._normalize_response(result, target_provider)
                else:
                    logger.warning(
                        f"Provider {target_provider} is unhealthy, trying fallbacks"
                    )
            except Exception as e:
        ...
        for fallback_provider in self.fallback_providers:
            if fallback_provider in self.providers and await self._is_provider_healthy(
                fallback_provider
            ):
                try:
                    logger.info(f"Using fallback provider {fallback_provider}")
                    result = await self.providers[fallback_provider].generate_recipe(
                        ingredients, model or ""
                    )
                    return self._normalize_response(result, fallback_provider)
                except Exception as e:
                    logger.error(
                        f"Fallback provider {fallback_provider} failed: {str(e)}"
                    )
                    continue
